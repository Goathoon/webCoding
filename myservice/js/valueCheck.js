$(document).ready(function(){
  ...
  loginSubmit.click(function(){
    //이메윌 유효성 검사식입니다. 이 식은 정규식이라고 합니다.
    //PHP에서 학습 예정 그냥 따라하셈
    var regEmailPattern = /^[a-zA-Z_\-0-9]+@[a-z]+.[a-z]+$/;
    //아래 if문의 조건문은 위의 정규식을 이메일 주소가 따르는지 아닌지를 판별
    //앞에 !가 있는 것에 주의할 것.
  ...
  //필터링 시 값이 오류일떄 무엇이 오류인지 보여주는 박스
  ...
  signUpSubmit.click(function(){
    ...
    if(userName.val() == ''){
      ...
      //이름 입력란에 포커스
      userName.focus();
      //오류 메시지를 2초후 사라지게 하는 함수를 호출
      timeOutCall();
      return false;
    }

    //값이 공백이 아니면 해당 값이 유효한 값인지 확인
    //아래의 식은 PHP 학습 시 학습하므로 그냥 따라해주세요.
    ...
    if($(":input:radio[name=gender]").val() == 'm' || $(":input:radio[name=gender]:checked").val()=='w'){
    ...
    //여기까지 return false 에 만나지 않았따면 true를 반환해 회원가입 정보를 제출
    return true;
  });
});

function timeOutCall(){
  //setTimeout 함수는 어떠한 기능의 시간을 지연시킬 떄 사용하는 내장 함수
  setTimeout(function(){
    $('#valueError').text('');
  },2000); //2초후 꺼짐.
}
